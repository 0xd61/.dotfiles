;
; dgl emacs config based on the Handmade Hero config of Casey
;
...
(setq dgl-dot-emacs "/home/dgl/.emacs.dgl.el")
...
; Precompile the .emacs file
  (require 'bytecomp)
  (setq compiled-dot-emacs (byte-compile-dest-file dgl-dot-emacs))
  (if (or (not (file-exists-p compiled-dot-emacs))
  	(file-newer-than-file-p dgl-dot-emacs compiled-dot-emacs)
          (equal (nth 4 (file-attributes dgl-dot-emacs)) (list 0 0)))
      (load dgl-dot-emacs)
    (load compiled-dot-emacs))

  (add-hook 'kill-emacs-hook
            '(lambda () (and (file-newer-than-file-p dgl-dot-emacs compiled-dot-emacs)
                             (byte-compile-file dgl-dot-emacs))))
;update
; Startup hook
