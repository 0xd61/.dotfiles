...
#define MAX_TAGS sizeof(uint32_t)*8
...
static void layout_handle_layout_demand (void *data, struct river_layout_v3 *river_layout_v3,
    ...
    if(output->monocle) {
        for (uint32_t i = 0; i < view_count; i++) {
            river_layout_v3_push_view_dimensions(output->layout, 0, 0, width, height, serial);
        }
        char label[16];
        snprintf(label, sizeof(label), "[%u]", view_count);
        river_layout_v3_commit(output->layout, label, serial);
    } else {
        ...
        const char *_command)
{
    ...
    if (word_comp(command, "main_count"))
    ...
    else if (word_comp(command, "layout")) {
        uint32_t cursor = strlen("layout") + 1;

        if(cursor < strlen(command)) {

            if(word_comp(command + cursor, "monocle")) {
                output->monocle = true;
            } else {
                output->monocle = false;
            }
        }
    }
        ...
        output->view_padding  = 2;
        output->outer_padding = 0;
        output->monocle       = false;
    }
    else
